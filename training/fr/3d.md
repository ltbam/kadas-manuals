...
Les couches 2D sont dessinées par défaut en tant que texture sur le globe. Les couches vectorielles (y compris redlining) peuvent également être représentées soit comme modèles extrudés ou 2.5D comme en modèles 3D. Le style de la vue 2D est dans la mesure du possible transféré dans la vue 3D. Pour l'affichage des données en 3D, les géométries doivent être pourvues d'une information de hauteur (coordonnée Z), et elles doivent être exprimées soit par rapport au niveau du sol ou de la mer. Les options d'affichage pour les niveaux vectoriels peuvent être définies dans les propriétés de couche correspondantes.
...
Les pin-points, les images de caméra et les symboles MSS ponctuels sont affichés sous forme de panneaux d'affichage.
